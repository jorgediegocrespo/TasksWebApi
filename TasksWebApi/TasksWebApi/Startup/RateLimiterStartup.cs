...
public class CustomRateLimiterPolicy : IRateLimiterPolicy<string>
{
    ...
    public CustomRateLimiterPolicy(ILogger<CustomRateLimiterPolicy> logger, IConfiguration configuration)
    {
    ...
    {
        var user = httpContext.GetContextUser();
        return user == null ? GetPartitionByClientIp(httpContext) : GetPartitionByUserId(user);
    }
    ...
    private RateLimitPartition<string> GetPartitionByClientIp(HttpContext httpContext)
    {
        return RateLimitPartition.GetSlidingWindowLimiter(httpContext.GetClientIp(),
            _ => new SlidingWindowRateLimiterOptions
            {
